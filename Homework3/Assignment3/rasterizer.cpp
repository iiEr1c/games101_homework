...
/**
 * 重心坐标不能直接套用作业2的公式, 会出现inf的情况
 * https://games-cn.org/forums/topic/guanyuzuoye5qiuzhongxinzuobiao/
 */
/*
...
static std::tuple<float, float, float> computeBarycentric(float x, float y,
                                                          const Vector4f *v) {
  // ppt中的ABC对应v[0]/v[1]/v[2]
  float xA = v[0].x(), xB = v[1].x(), xC = v[2].x();
  float yA = v[0].y(), yB = v[1].y(), yC = v[2].y();
...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
  ...
  for (const auto &t : TriangleList) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      //  将mvp变换+到屏幕空间的变换的结果 覆盖原来三角形的xyzw
...
void rst::rasterizer::rasterize_triangle(
  ...
  for (const auto &k : t.v) {
    float x = k.x(), y = k.y();
    leftBound = std::min(leftBound, x);
    upBound = std::min(upBound, y);

    rightBound = std::max(rightBound, x);
    buttonBound = std::max(buttonBound, y);
  }
  // bounding box end
  int horizontalMin = std::floor(leftBound),
      horizontalMax = std::ceil(rightBound);
  int verticalMin = std::floor(upBound), verticalMax = std::ceil(buttonBound);

  // std::array<std::array<float, 2>, 4> direction = {0.25, 0.25, 0.25, 0.75,
  //                                                  0.75, 0.25, 0.75, 0.75};
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h) {
  ...
  // for msaa
  depth_buf.resize(w * h * msaa_size * msaa_size);

  texture = std::nullopt;
}

// 这里的get_index和set_pixel都是将以左下脚为起点的xy坐标系转换为左上角为起点的xy坐标系
// (0, 0) -> (height - 1, 0)
...
int rst::rasterizer::get_msaa_index(int x, int y, int offset) {
  ...
  return y * width * scale + x * scale + offset;
}
